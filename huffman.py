...
class Huffman:
    ...
    def generar_codificacion_huffman(self, arbol_huffman, prefijo="", codificacion={}):
        if arbol_huffman.caracter is not None:
            codificacion[arbol_huffman.caracter] = prefijo
        if arbol_huffman.izquierda is not None:
            self.generar_codificacion_huffman(arbol_huffman.izquierda, prefijo + "0", codificacion)
        if arbol_huffman.derecha is not None:
            self.generar_codificacion_huffman(arbol_huffman.derecha, prefijo + "1", codificacion)

    def codificar_mensaje(self, mensaje, codificacion):
        mensaje_codificado = ""
        for caracter in mensaje:
            mensaje_codificado += codificacion[caracter]
        return mensaje_codificado

    def decodificar_mensaje(self, mensaje_codificado, arbol_huffman):
...
codificacion = {}
huffman.generar_codificacion_huffman(arbol_huffman, "", codificacion)
mensaje_codificado = huffman.codificar_mensaje("mensaje", codificacion)
